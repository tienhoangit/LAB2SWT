...
public class cart extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int cartId = dao.getCartById(id).getCid();
        Cart c = dao.getCartById(id);
        if (c == null || (dao.getPC(c.getCid()).isEmpty())) {
            String msError = "No Products Here !";
            request.setAttribute("msError", msError);
        } else {

            float total = 0;

            ArrayList<ProductCart> list = dao.getPC(c.getCid());
            for (ProductCart i : list) {
                total += i.getTotal();

            }
            DecimalFormat df = new DecimalFormat("#.00");
            String formattedTotal = df.format(total);
            request.setAttribute("userID", cartId);
